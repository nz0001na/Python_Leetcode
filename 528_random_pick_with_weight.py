...
'''
******************************************************************************
Solution 1:  for input w, create accumulate sum list.
           generate random numbers between [1, sum]
           check which accumulate sum range of this random number in (O(n) search)
'''
...
class Solution:
    ...
    def pickIndex(self) -> int:
        ...
        for i in range(len(self.w)):
            s += self.w[i]
            sums.append(s)
        n = random.randint(1, sums[-1])
        if n <= sums[0]:
            return 0
        for i in range(1, len(sums)):
            if n > sums[i - 1] and n <= sums[i]:
                return i
...
Solution 1:  for input w, create accumulate sum list.
           generate random numbers between [1, sum]
           check which accumulate sum range of this random number in using binary search
'''
